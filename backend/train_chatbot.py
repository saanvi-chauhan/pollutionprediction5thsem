...
def main():
    ...
    with open('../data/chatbot/intents.json', 'r', encoding='utf-8') as f:
        data = json.load(f)
    ...
    print(f"[RESULT] Cross-Val Score: {cv_scores.mean() * 100:.2f}% (+/- {cv_scores.std() * 2 * 100:.2f}%)")
    ...
    shutil.copy('../data/chatbot/intents.json', models_dir / 'chatbot_intents.json')
